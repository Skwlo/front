...
export default {
  ...
  data() {
    return {
      articles: {},
      user: localStorage.getItem("user") ? JSON.parse(localStorage.getItem("user")) : null, // 获取当前登录人的信息
      comments: [],
      commentForm: {},
  ...
  methods: {
    ...
    save() {  // 保存评论 content
      ...
      this.request.post("/comment", this.commentForm).then(res => {
        if (res.code === '200') {
          ...
          this.commentForm = {};  // 初始化评论对象 content
          this.loadComment();
    ...
    collect(content) {  // 收藏
      if (!this.user) {
        this.$message.warning("请先登录再进行收藏");
        return;
      }
      request.post("/collect/", {
        userId: this.user.id,
        title: this.articles.title,
        url: this.articles.id,
      }).then(res => {
        if (res.code === '200') {
          this.$message({
            message: "收藏成功",
